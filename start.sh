...
# 2. SFTP-SERVER FIX
# ══════════════════════════════════════
echo -e "${C}[2/7]${W} SFTP-server beállítása...${N}"

# Keresés
SFTP_BIN=$(find /usr -name "sftp-server" -type f 2>/dev/null | head -1)
if [ -z "$SFTP_BIN" ]; then
    echo -e "  ${R}✗${N} sftp-server NEM TALÁLHATÓ!"
else
    echo -e "  ${G}✓${N} sftp-server: ${SFTP_BIN}"
    # Symlink minden lehetséges helyre
    mkdir -p /usr/libexec 2>/dev/null || true
    ln -sf "$SFTP_BIN" /usr/libexec/sftp-server 2>/dev/null || true
    ln -sf "$SFTP_BIN" /usr/local/bin/sftp-server 2>/dev/null || true
    ln -sf "$SFTP_BIN" /usr/bin/sftp-server 2>/dev/null || true
    ln -sf "$SFTP_BIN" /usr/lib/sftp-server 2>/dev/null || true
    chmod +x "$SFTP_BIN" 2>/dev/null || true
    echo -e "  ${G}✓${N} Symlink-ek létrehozva"
fi

# Dropbear SFTP beállítás — /etc/shells fix
grep -q "/bin/bash" /etc/shells 2>/dev/null || echo "/bin/bash" >> /etc/shells
grep -q "/bin/sh" /etc/shells 2>/dev/null || echo "/bin/sh" >> /etc/shells
echo -e "  ${G}✓${N} /etc/shells OK"

# ══════════════════════════════════════
# 3. NGINX
# ══════════════════════════════════════
echo -e "${C}[3/7]${W} Nginx konfiguráció...${N}"
...
# 4. BORE WRAPPER
# ══════════════════════════════════════
echo -e "${C}[4/7]${W} bore tunnel wrapper...${N}"
...
# 5. SFTP FRISSÍTŐ
# ══════════════════════════════════════
echo -e "${C}[5/7]${W} SFTP frissítő...${N}"
...
# ══════════════════════════════════════
# 6. PERSISTENT PROCESS KEZELŐ
# ══════════════════════════════════════
echo -e "${C}[6/7]${W} Persistent Process Kezelő...${N}"
...
if [ -z "$SESSIONS" ]; then
    ...
    echo "  Példa:   bg-start mybot 'python3 bot.py'"
    echo ""
else
    echo "$SESSIONS" | while read -r line; do
        ...
        CMD_TEXT=""
        [ -f "$CMD_FILE" ] && CMD_TEXT=$(cat "$CMD_FILE")
        echo -e "  \033[1;32m●\033[0m \033[1;37m${SESS_NAME}\033[0m"
        [ -n "$CMD_TEXT" ] && echo -e "    Parancs: \033[1;33m${CMD_TEXT}\033[0m"
        echo ""
    done
...
#!/bin/bash
if [ $# -lt 1 ]; then
    echo "Használat: bg-logs <név> [sorok|-f]"
    exit 1
fi
NAME="$1"
LINES="${2:-50}"
...
if [ ! -f "$CMD_FILE" ]; then
    echo -e "\033[1;31m✗ Nincs mentett parancs '${NAME}' számára.\033[0m"
    exit 1
fi
CMD=$(cat "$CMD_FILE")
echo "Leállítás..."
...
if [ -z "$SESSIONS" ]; then
    ...
    echo -e "\n\033[1;32m✅ Minden leállítva.\033[0m"
fi
echo ""
BGSTOPALL
...
# 7. SEGÉDPARANCSOK
# ══════════════════════════════════════
echo -e "${C}[7/7]${W} Segédparancsok...${N}"
...
echo "  sftp:     $(ls -la /usr/lib/openssh/sftp-server 2>/dev/null && echo 'OK' || echo 'HIÁNYZIK')"
echo ""
...
cat > /usr/local/bin/vps-sftp-fix << 'SFTPFIX'
#!/bin/bash
echo ""
echo "═══════════════════════════════"
echo "  SFTP JAVÍTÁS"
...
# sftp-server keresés
echo "[1] sftp-server keresés..."
SFTP_BIN=$(find /usr -name "sftp-server" -type f 2>/dev/null | head -1)
if [ -n "$SFTP_BIN" ]; then
    echo "  Megtalálva: $SFTP_BIN"
    echo "  Symlink-ek:"
    mkdir -p /usr/libexec 2>/dev/null || true
    ln -sf "$SFTP_BIN" /usr/libexec/sftp-server 2>/dev/null && echo "    ✓ /usr/libexec/sftp-server"
    ln -sf "$SFTP_BIN" /usr/local/bin/sftp-server 2>/dev/null && echo "    ✓ /usr/local/bin/sftp-server"
    ln -sf "$SFTP_BIN" /usr/bin/sftp-server 2>/dev/null && echo "    ✓ /usr/bin/sftp-server"
else
    echo "  ✗ NEM TALÁLHATÓ! Telepítés..."
    apt-get update && apt-get install -y openssh-sftp-server
fi

# dropbear újraindítás
echo ""
echo "[2] dropbear újraindítás..."
supervisorctl restart dropbear 2>/dev/null || true
sleep 2
pgrep -x dropbear >/dev/null && echo "  ✓ dropbear fut" || echo "  ✗ dropbear NEM fut"

# bore ellenőrzés
echo ""
echo "[3] bore tunnel..."
pgrep -f "bore local" >/dev/null && echo "  ✓ bore fut" || echo "  ✗ bore NEM fut"

echo ""
echo "FileZilla beállítások:"
BORE_PORT="${BORE_PORT:-48251}"
echo "  Host: bore.pub"
echo "  Port: ${BORE_PORT}"
echo "  Protokoll: SFTP"
...
echo "  Pass: 2003"
echo ""
echo "Ha még nem megy: próbáld SSH-val (Putty/terminál) először."
echo "Ha SSH működik de SFTP nem: vps-tunnel-restart"
echo ""
SFTPFIX
chmod +x /usr/local/bin/vps-sftp-fix
...
if [ -z "$VPS_GREETED" ]; then
    ...
    echo -e "\033[1;36m  ═════════════════════════════════════════\033[0m"
    echo ""
    echo -e "\033[1;37m  Alap:\033[0m"
    echo -e "\033[1;33m    vps-info\033[0m              → Infó"
    echo -e "\033[1;33m    vps-tunnel-restart\033[0m    → Tunnel újra"
    echo -e "\033[1;33m    vps-sftp-fix\033[0m          → SFTP javítás"
    echo ""
    echo -e "\033[1;37m  Háttérfolyamatok:\033[0m"
...
<div class="row">
    <div class="card full">
        ...
        <p class="info">Teljes root shell — nem kell semmi telepíteni!</p>
    </div>
</div>
...
<div class="row">
    <div class="card full">
        ...
        <p class="info">Drag & drop fájlfeltöltés!</p>
    </div>
</div>
...
<div class="row">
    <div class="card full">
        <h2>📚 Parancsok</h2>
        <pre>═══ ALAP ═══
vps-info              # Rendszer infó
vps-tunnel-restart    # Tunnel újra
vps-sftp-fix          # SFTP javítás

═══ HÁTTÉRFOLYAMATOK ═══
bg-start mybot 'python3 bot.py'
bg-list / bg-logs mybot / bg-attach mybot
bg-stop mybot / bg-restart mybot / bg-stopall

═══ RENDSZER ═══
htop / ll / free -h / df -h</pre>
    </div>
</div>
</div>
...
function load(){
    fetch('/sftp.txt').then(r=>r.text()).then(t=>{
        if(t.includes('AKTIV')){
            ...
            if(host&&port){
                document.getElementById('ssh-info').textContent='SSH parancs:\n  ssh root@'+host+' -p '+port+'\n\nJelszó: 2003\n\nPuTTY:\n  Host: '+host+'\n  Port: '+port+'\n  User: root\n  Pass: 2003';
                document.getElementById('sftp-info').textContent='Protocol: SFTP\nHost: '+host+'\nPort: '+port+'\nUser: root\nPass: 2003\n\nHa nem megy: ssh-ból futtasd:\n  vps-sftp-fix';
            }
        } else {
...
echo ""
echo -e "  ${C}Ha SFTP nem megy:${N} ${G}vps-sftp-fix${N}"
echo ""
echo -e "${C}════════════════════════════════════════════════════════${N}"
echo ""
